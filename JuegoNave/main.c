...
int main(void) {
  ...
  if (SDL_Init(SDL_INIT_EVERYTHING) < 0) {
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "OK", "SDL started",
                             NULL);
  }
  ...
  window =
      SDL_CreateWindow("Ship", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
...
void ShipDraw(Ship *ship, SDL_Renderer *renderer) {
  ...
  while (ixMissile) {
    ...
    SDL_RenderDrawLine(renderer, ixMissile->x1, ixMissile->y1, ixMissile->x2,
                       ixMissile->y2);
  ...
  // draw ship
  SDL_Point points[4] = {
      {ship->x1, ship->y1}, {ship->x2, ship->y2}, {ship->x3, ship->y3}};
  points[3] = points[0];
